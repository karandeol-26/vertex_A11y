...
function scanPage(){
  ...
  for (const img of imgs) {
    ...
    const hasAlt = img.hasAttribute("alt") && String(img.getAttribute("alt")).trim().length > 0;
    if (!hidden && !hasAlt) {
      issues.push(issue("Images", "high",
        "Image missing alt text. Add a concise description of what the image conveys.",
        outerHTMLSnippet(img),
        `Add an alt: <img src="${(img.getAttribute("src")||"").split("?")[0]}" alt="describe image">`,
        pathOf(img), true));
    } else {
      passed++;
    }
  ...
  for (const el of textNodes) {
    ...
    if (ratio < min) {
      issues.push(issue("Contrast", "high",
  ...
  for (const el of clickCandidates) {
    ...
    issues.push(issue("Keyboard", "medium",
  ...
  for (const tag of landmarks) {
    ...
    if (!exists) {
      issues.push(issue("Semantic", "low",
        `Missing landmark <${tag}>.`,
        "", `Add a <${tag}> landmark to improve screen reader navigation.`, null, false));
    } else {
      passed++;
    }
  }
  ...
  if (headings.length) {
    ...
    for (const h of headings) {
      ...
      if (last && lvl > last + 1) { ok = false; break; }
      last = lvl;
    }
    if (!ok) {
      issues.push(issue("Semantic", "low",
        "Heading levels skip order (e.g., H2 directly to H4).",
        "", "Use headings without skipping levels to preserve structure.", null, false));
    } else passed++;
  }

  // 5) Form labels
  const inputs = Array.from(document.querySelectorAll("input,textarea,select"));
  const inputsVisible = inputs.filter(el => (el.type||"") !== "hidden" && isVisible(el));
  checked += inputsVisible.length || 1;
  for (const el of inputsVisible) {
    const ok = hasAccessibleLabel(el);
    if (!ok) {
      issues.push(issue("Forms", "high",
  ...
  const zoomBlocked = metaV && /user-scalable\s*=\s*no|maximum-scale\s*=\s*1(\.0+)?(\b|,|;)/i.test(metaV.content||"");
  if (zoomBlocked) {
    issues.push(issue("Zoom", "high",
...
function issue(type, severity, message, snippet, tip, path, fixable){
  return { id: cryptoRandom(), type, severity, message, snippet, tip, path, fixable: !!fixable };
}
function cryptoRandom(){ try{ return crypto.getRandomValues(new Uint32Array(1))[0].toString(16); }catch{ return String(Math.random()).slice(2); } }
...
function hasAccessibleLabel(el){
  ...
  if (aria && aria.split(/\s+/).some(id => document.getElementById(id))) return true;
  return false;
}
...
function keyboardCandidates(){
  ...
  return all.filter(el => {
    const cs = getComputedStyle(el);
    const looksClickable = cs.cursor === "pointer" || el.getAttribute("role") === "button" || el.hasAttribute("onclick");
...
function textElementsSample(){
  ...
  const nodes = Array.from(document.querySelectorAll("p,li,span,small,button,a,div,h1,h2,h3,h4,h5,h6"));
