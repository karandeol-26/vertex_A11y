...
async function runScan() {
  ...
  try {
    ...
    if (!/^https?:\/\//i.test(tab.url || "")) {
      throw new Error("This page type blocks extensions. Open a normal http(s) website and try again.");
    }

    let report;
    try {
      report = await chrome.tabs.sendMessage(tab.id, { type: "VERTEX_SCAN" });
      if (!report || report.error) throw new Error(report?.error || "Scan failed.");
    } catch {
      // classic injection (content.js is a plain script)
      await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ["content.js"] });
      report = await chrome.tabs.sendMessage(tab.id, { type: "VERTEX_SCAN" });
      if (!report || report.error) throw new Error(report?.error || "Scan failed after injection.");
    }
...
function applyFiltersAndSort(items){
  ...
  if (by === "type")     arr.sort((a,b)=>(a.type||"").localeCompare(b.type||""));
  if (by === "severity") arr.sort((a,b)=>severityRank(b.severity)-severityRank(a.severity));
...
issuesList.addEventListener("click", async (e) => {
  ...
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.id) throw new Error("No active tab.");
    if (!/^https?:\/\//i.test(tab.url || "")) throw new Error("Action blocked on this page.");
...
function renderIssues(items){
  ...
  for (const it of items){
    ...
    if ((it.type || "").toLowerCase() === "images") {
      ...
      if (src) previewHTML = `<div class="preview"><img src="${escapeAttr(src)}" alt=""></div>`;
    }

    el.innerHTML = `
      <div class="head">
        <div class="type">${escapeHTML(it.type || "Issue")}</div>
        <div class="sev ${escapeHTML(it.severity || "low")}">${escapeHTML(it.severity || "low")}</div>
      </div>
      <div class="muted" style="margin-top:4px">${escapeHTML(it.message || "")}</div>
      ${it.snippet ? `<pre class="code">${escapeHTML(it.snippet)}</pre>` : ""}
      ${it.tip ? `<div style="margin-top:6px">💡 <strong>Fix:</strong> ${escapeHTML(it.tip)}</div>` : ""}
      ${previewHTML}
      <div class="actions">
        <button class="btn" data-action="reveal" data-path="${escapeAttr(it.path || "")}">Reveal on page</button>
      </div>
    `;
...
function renderError(err){
  ...
  scoreCard.innerHTML = `
    ...
    <div class="muted">${escapeHTML(err?.message || "Something went wrong.")}</div>
  `;
}
